...
- 做了什么：用 A/C 明确结果样本，按品牌聚合 A_count、C_count、c_to_a_ratio、brand_share_in_clear_outcome，用中位数切分四象限
- 结果：
  - 合格品牌 98 个（排除 Unknown Brand 1个 + 样本不足 124个 + 无购买记录 10个）
  - 核心健康品牌 22 个（高体量×低风险）：runail, irisk, bpw.style
  - 重点治理品牌 27 个（高体量×高风险）：masura, grattol, ingarden
  - 潜力/普通长尾品牌 27 个（低体量×低风险）：kaaral, benovy, skinlite
  - 问题长尾品牌 22 个（低体量×高风险）：shik, beauty-free, ecolab
- 解读：
  - 核心健康品牌：给流量就能转化，是 GMV 确定性来源
  - 重点治理品牌：体量大但流失严重，需要加购挽回弹窗、限时折扣等干预手段
  - 潜力长尾：转化健康但量小，可试探性增加曝光观察能否放量
  - 问题长尾：量小且流失高，优先级最低，考虑降低推荐权重

## 2026-07-05 — 48小时观察窗口口径修正

- **背景问题**：复盘 `03_user_behavior_groups.csv` 时发现时间窗口口径风险。原表按 2019 年 12 月事件聚合，但月末首次加购样本可能没有完整后续观察期；同时部分 A/C 记录可能是 12 月内购买或移除，但真实加购发生在 12 月之前。

- **新口径设计**：新增 48 小时固定观察窗口版本，避免不同日期样本拥有不同长度的后续观察期。
  - 一行 = `user_session × product_id` 的首次真实 `cart`
  - 只保留 `first_cart_time < 2019-12-30 00:00:00+00`
  - 每条样本只观察首次 `cart` 后 48 小时
  - 48 小时内出现 `purchase` → A
  - 没有 `purchase` 但出现 `remove_from_cart` → C
  - `purchase` / `remove_from_cart` 都没有 → B
  - 如果窗口内既有 remove 又有 purchase，按最终成交优先，归为 A
...
- **新表质检结果**：
  - 新表 `data/interim/11_user_behavior_groups_window_48h.csv` 共 772,489 行
  - A：104,696，占 13.55%
  - B：511,656，占 66.23%
  - C：156,137，占 20.21%
  - 重复 `user_session × product_id`、非 cart 起点、窗口长度错误、A/B/C 标签逻辑错误均为 0

- **与旧口径对比**：
  - 旧口径 A/B/C 占比分别为 18.34% / 44.92% / 36.74%
  - 48 小时口径 A/B/C 占比分别为 13.55% / 66.23% / 20.21%
  - 新口径下 B 组占比明显上升，说明首次加购后 48 小时内，更多样本表现为“暂时没有购买也没有移除”
  - A/C 下降说明旧口径中的一部分购买或移除结果不一定发生在首次加购后的短窗口内，可能来自更长决策周期或历史购物车状态

- **品牌结论稳健性检查**：
  - 48 小时窗口下，各大品牌 C/A 普遍下降
  - `masura` 新 C/A = 2.3093，`runail` 新 C/A = 1.3548，`masura` 仍显著高于 `runail`
  - 中位数风险阈值在新口径下降到 1.268987，导致 `runail` / `irisk` 被相对分层误分到高风险
  - 因此新增固定阈值版本：`C/A >= 1.5` 记为高风险
...
## 2026-07-07 — Excel 与 Power BI 交付练习

- **Excel 交付练习**
  - 基于 `reports/11_brand_quadrant_matrix_window_48h.csv` 整理了一版 Excel 交付物：`reports/eCommerce_brand_quadrant_excel_practice.xlsx`
  - 没有把原始事件明细表导入 Excel，而是使用已经聚合好的品牌四象限结果；当前理解是：SQL/Python 更适合完成清洗、聚合和口径固化，Excel 更适合做轻量交付、字段解释、透视表和业务沟通。
  - 完成了三类 Sheet：
    - `01_口径说明`：写清楚分析粒度 `user_session × product_id`、48 小时观察窗口、A/B/C 分组、明确结果购买率、C/A 风险比和品牌体量定义。
    - `02_品牌明细表`：把分析字段改成业务可读字段，并补充风险等级和建议动作。
    - `03_品牌分析透视表`：按品牌象限和风险等级汇总品牌数、购买样本数、移除样本数、明确结果样本数和风险指标。
  - 当前认识：Excel 交付的重点不是把图表做复杂，而是让业务方能看懂字段、知道指标怎么算、能筛选重点品牌，并看到每个品牌对应的建议动作。

- **Power BI 看板启动**
  - 在 `reports/first_brand.pbix` 中导入了 Excel 文件的 `02_品牌明细表`。
  - 明确了 Power BI 不应该读取 Excel 里的透视表或 Dashboard 页，而应该读取结构化明细表；透视表和摘要页是展示结果，不适合作为 BI 的事实数据源。
  - 学习了 Power BI 中“列”和“度量值”的区别：
    - Excel 明细表里的 `明确结果购买率`、`C/A风险比` 是单个品牌的行级结果。
    - Power BI 看板卡片需要的是当前筛选条件下的整体结果，应该用 DAX Measure 动态计算。
  - 核心 Measure 口径：
  ...
  - 当前理解：整体购买率不能直接平均品牌购买率，整体 C/A 风险比也不能直接平均品牌 C/A；正确做法是先 `SUM` 分子和分母，再用 `DIVIDE` 得到当前筛选上下文下的指标。

- **口径认识更新**
  - 分析口径关注：样本范围、时间窗口、数据粒度、指标定义和边界情况。
  - 交付口径关注：字段是否业务可读、指标是否能被正确汇总、切片器筛选后 KPI 是否动态重算、样本量过小时是否提醒。
  - Power BI 里的切片器本质上是在改变筛选上下文，Measure 会在新的上下文下重新计算；这比 Excel 静态表更适合做可交互看板。
  - 当前阶段的重点不是做很复杂的视觉设计，而是把核心 KPI、切片器和品牌明细表搭起来，保证所有数字口径一致。

## 2026-07-13 — 企业化正式链路补全

- **本阶段目标**：保留原有学习轨迹，同时补齐一条从业务立项、数据质量、指标口径、SQL、Python、Notebook、Excel、管理层报告到 Power BI 实施规范的可复现正式链路。
- **正式业务问题**：不再把品牌直接定义为“导致流失的风险品牌”，而是用现有数据确定第一批运营排查对象，并明确后续需要 SKU 下钻、补充字段或实验验证。
- **正式队列**：
  - 粒度：有效 `user_session × product_id` 的首次观测加购。
  - 窗口：首次加购后完整 48 小时，UTC。
  - 样本：772,119；A/B/C = 104,696 / 511,286 / 156,137。
  - 总体购买率 13.5596%，未明确处置率 66.2185%，移除率 20.2219%。
  - 已知品牌覆盖率 55.9880%，因此品牌结论只代表已知品牌子样本。
- **关键修复**：
  - 将字面值 `user_session='NaN'` 和品牌占位值标准化为无效/未知，避免伪会话串联和品牌榜失真。
  - 排除 19,231 个无法完整观察 48 小时的尾部候选。
  - 购买优先于移除，保证 A/B/C 互斥；标签矛盾为 0。
  - 窗口敏感性使用同一批 756,116 个完整 72 小时样本，避免分母变化。
  - 将 `C/A risk ratio` 收紧为“移除/购买比”，明确它不是概率、因果风险或行业标准。
- **验证结果**：SQL 与 Python 共 25 项检查为 0 个错误；唯一警告是旧 CSV 多出 369 个无效会话样本，已定位原因并保留旧文件作为学习轨迹。
- **新增正式成果**：
  - `docs/`：立项书、数据字典、指标字典、成果清单、优化记录、环境复现和面试表达。
  - `sql/formal/`：源概况、队列质量、KPI、品牌、漏斗、窗口敏感性和队列明细。
  - `scripts/`：正式执行与对账、图表/报告源、Notebook 和管理层报告快照构建。
  - `notebooks/11_正式分析与验证.ipynb`：已完整执行，无错误输出。
  - `outputs/formal_delivery/eCommerce_brand_priority_formal.xlsx`：管理层总览、品牌优先级、窗口、对账质量和口径页。
  - `reports/management_report_artifact.json`：已通过报告结构与数据来源校验。
  - `reports/power_bi_formal_spec.md` 与 `reports/power_bi_measures.dax`：两页看板和筛选上下文下的重算口径。
- **未完成但已明确边界**：当前机器未发现 Power BI Desktop，因此没有新建或伪造 PBIX；旧 PBIX 保留。后续在可用环境中按规范搭建并执行切片器 QA。
